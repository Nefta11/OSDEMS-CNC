...
const SocialMedia = ({ 
  ...
  if (isLoading) {
    return <div className={styles.loading}>Cargando redes sociales...</div>;
  }

  return (
    <div className={`${styles.socialMedia} ${compact ? styles.compact : ''}`}>
      {links.map((link, index) => (
        <a
          ...
          title={`Síguenos en ${link.platform}`}
        >
          {/* Iconos SVG para mejor compatibilidad */}
          ...
          <span className={styles.socialText}>{link.platform}</span>
        </a>
      ))}
    </div>
  );
